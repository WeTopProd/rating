...
export default function PoiksRezume ({onClick, setAuth, auth, mycardId}) {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost/api/resume/?&is_active=true', {
  ...
  const PoiskValueVakan = (event) => {
    ...
    // http://localhost/api/resume/?post_work=${poiskvalue}&city=${city}&education=&start_salary=${income}&final_salary=${income}&is_favorited=

    axios.get(`http://localhost/api/resume/?city=${city}&post_work=${poiskvalue}&company_name=&start_salary=${income}&final_salary_min=${income}&final_salary_max=&start_experience_min=&start_experience_max=&final_experience_min=&final_experience_max=&employment_type=&is_favorited=&salary_min=${income}&salary_max=&is_active=true`,{
